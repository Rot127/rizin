...
bool sdb_load_base_types(RzTypeDB *typedb, Sdb *sdb) {
	...
	ls_foreach (l, iter, kv) {
		...
		if (tpair && tpair->type) {
			...
			// After base type loaded, we should also convert it to
			// the format to precalculate formats for all loaded types
			// If the SDB provided the desired format then we use it instead
			char *format = tpair->format ? strdup(tpair->format) : strdup(rz_base_type_as_format(typedb, tpair->type));
			ht_pp_insert(typedb->types, tpair->type->name, format);
