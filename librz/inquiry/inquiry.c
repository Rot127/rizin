...
RZ_API bool rz_inquiry_get_fcn_symbol_addr(RzCore *core, RZ_OUT RzSetU *symbol_targets) {
	...
	RzPVector /*<RzBinSection *>*/ *sections = rz_bin_object_get_segments(core->bin->cur->o);
	if (!sections) {
		rz_warn_if_reached();
		return false;
	}
...
RZ_API bool rz_inquiry_interpreter(RzCore *core,
	...
	RzILCache *il_cache = rz_il_cache_new(core->analysis, core->io,
		rz_bin_object_get_segments(core->bin->cur->o),
	...
	if (!setup_yield_rbufs(yield_rbufs, rz_bin_object_get_segments(core->bin->cur->o),
