...
static bool buf_init(RzBuffer *b, const void *user) {
	const BufInitUser *iu = user;
	BufCtx *ctx = RZ_NEW0(BufCtx);
	if (!ctx) {
		return false;
	}
	ctx->obj = iu->obj;
	ctx->boffset = iu->boffset;
	b->priv = ctx;
	return true;
}
...
static st64 buf_read(RzBuffer *b, ut8 *buf, ut64 len) {
	...
	rebase_buffer(ctx->obj, ctx->off - ctx->boffset, buf, RZ_MIN(r, len));
	return r;
}
...
RZ_API RzBuffer *MACH0_(new_rebasing_and_stripping_buf)(struct MACH0_(obj_t) * obj, ut64 boffset) {
	BufInitUser iu = {
		.obj = obj,
		.boffset = boffset
	};
	return rz_buf_new_with_methods(&buf_methods, &iu);
}
