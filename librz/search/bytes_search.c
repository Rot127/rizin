 ...
 *
 * The given byte pattern string should be of the form: `<mask>:<bytes>`.
 *
 * Formatting rules:
 ...
 *  - `ff0f:a907`: Pattern with a mask of `ff0f` and bytes of '\xa9\x07'.
 *  - `0xff0f:0xa907`: Mask and bytes with "0x" prefixes.
 */
RZ_API RZ_OWN RzSearchBytesPattern *rz_search_parse_byte_pattern(const char *byte_pattern, RZ_NULLABLE const char *pattern_desc) {
	...
	RzRegexMatch *mask_match = rz_pvector_at(matches, 3);
	RzRegexMatch *bytes_match = rz_pvector_at(matches, 5);
	if (!mask_match || !bytes_match) {
